...
from scripts.utils import CalcTopMap, compute_result, pr_curve, p_top
...
def train_val(config, bit, l):
    ...
    for epoch in range(config['epoch']):
        ...
        for img, label, ind in tqdm(train_loader):
            img = img.cuda()
            label = label.cuda()   #cifar10数据集时用float()，其他直接使用 label = label.cuda()
            optimizer.zero_grad()
        ...
        if (epoch + 1) % config['test_map'] == 0:
            ...
            if mAP > Best_map:
                ...
                if 'save_path' in config:
                    ...
                    # #保存trn_binary
                    # np.save(os.path.join(config["save_path"], str(bit), config["dataset"] + "-" + str(bit) + "-" + str(mAP) + "-trn_binary.npy"),
                    #         trn_binary.numpy())
                    # #保存tst_binary
                    # np.save(os.path.join(config["save_path"], str(bit), config["dataset"] + "-" + str(bit) + "-" + str(mAP) + "-tst_binary.npy"),
                    #         tst_binary.numpy())
                    # #保存trn_label
                    # np.save(os.path.join(config["save_path"], str(bit), config["dataset"] + "-" + str(bit) + "-" + str(mAP) + "-trn_label.npy"),
                    #         trn_label.numpy())
                    # #保存tst_label
                    # np.save(os.path.join(config["save_path"], str(bit), config["dataset"] + "-" + str(bit) + "-" + str(mAP) + "-tst_label.npy"),
                    #         tst_label.numpy())
                    # #保存model
    ...
    np.savetxt('./results/' + config["dataset"] + '/' + config["remarks"] + '/' + str(bit) + '/' + 'p.txt', P, fmt='%3.5f')
    np.savetxt('./results/' + config["dataset"] + '/' + config["remarks"] + '/' + str(bit) + '/' + 'r.txt', R, fmt='%3.5f')
    np.savetxt('./results//' + config["dataset"] + '/' + config["remarks"] + '/' + str(bit) + '/' + 'P_top.txt', P_top,
               fmt='%3.5f')
    writer.close()
